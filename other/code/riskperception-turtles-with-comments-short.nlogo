...
;; il setup si occupa di: creare la rete con l'ausilio della procedura create-network, inizializzare lo stato degli agenti e generare un numero casuale di infetti ;;
to setup
  clear-all
  no-display
  set-default-shape turtles "circle"
  create-turtles 2 [ fd 5 ]
  ask turtle 0 [ create-link-with turtle 1 ]
  repeat num-nodes [ create-network ]
  ask turtles [
    set state 0
    set infected? false ]
  ask turtles [
...
;; procedura che regola il processo di diffusione dell'infezione: ogni sua esecuzione corrisponde ad un incremento della variabile che tiene traccia del tempo ;;
to go
  ...
  ;; si vanno a contagiare, in modo casuale, gli agenti che hanno dei vicini infetti secondo una probabilita' che viene influenzata dalla risk-perception e dal loro livello di alert; l'infezione durera' per un numero di tick pari a num-states ;;
  ask turtles with [state > 0] [
    ask out-link-neighbors with [state = 0] [
...
;; metodo utilizzato nelle procedure setup e go per colorare gli agenti in base al loro stato di salute ;;
to recolor
  ask turtles [
    ifelse state = 0 [
      ifelse infected? [ set color yellow ][ set color green ]
	][ set color red ]]
end

;; metodo che viene chiamato nella procedura to go dove viene ripetuto per un numero di volte pari al numero di nodi della rete: seleziona in modo casuale uno degli agenti, al quale viene connesso un nuovo nodo appena generato ;;
to create-network
...
;; procedura che viene invocata dal metodo create-network e che si occupa di impostare l'aspetto grafico dei nodi e dei link ;;
to layout
  ask turtles [
